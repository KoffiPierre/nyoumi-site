...
const copy: Record<Locale, { label: string; title: string; lede: string; metaTitle: string; metaDesc: string }> = {
  fr: {
    ...
    lede: "Mathématicien, physicien et informaticien de formation, il voit l'écriture comme un prolongement de la recherche et de l'enseignement.",
    metaTitle: 'Ouvrage',
  ...
  en: {
    ...
    lede: 'A mathematician, physicist and computer scientist by training, he sees writing as an extension of research and teaching.',
    metaTitle: 'Book',
  ...
  return (
    <>
      ...
      {publications.map((pub, i) => (
        <Section key={pub.id} background={i % 2 === 0 ? 'paper' : 'alt'}>
          <Reveal>
            {i === 0 ? (
              <BookFeature publication={pub} locale={locale} />
            ) : (
              <div className={styles.articleCard}>
                <span className="eyebrow">{pub.kind}</span>
